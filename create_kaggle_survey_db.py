...
class CreateKaggleSurveyDB:
    def __init__(self):
        ...
        for survey_year in survey_years:
            ...
            df = df.iloc[:,1: ] #row 1 is the column name(question in csv)
    ...
    def tidy_2020_2021(self,survey_year:int)-> tuple:
        ...
        for column_name, question_description in zip(column_names, descriptions):
            ...
            questions_descriptions_split = question_description.split(" - ")
            # 單選題
            if(len(column_name_split) == 1):
                ...
                questions_descriptions.append(questions_descriptions_split[0])
            else:
                ...
                questions_descriptions.append(questions_descriptions_split[0])
                #print(questions_descriptions_split[0])


        question_df = pd.DataFrame()
        question_df["question_index"] = question_indexes
        question_df["question_type"] = question_types
        question_df["question_description"] = questions_descriptions
        ...
        response_df_melted["responsed_in"] = survey_year
    ...
    def create_database(self):
        ...
        create_view_sql = """
            ...
            SELECT 
                questions.surveyed_in,
                questions.question_index,
                questions.question_type,
                questions.question_description,
                responses.response,
                COUNT(responses.respondent_id) AS responce_count
            FROM questions
            JOIN responses
            ON questions.question_index = responses.question_index AND
                questions.surveyed_in = responses.responsed_in
            GROUP BY
                questions.surveyed_in,
                questions.question_index,
                responses.response;
